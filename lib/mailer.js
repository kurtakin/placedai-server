 ...
 */
/**
 * HTML metin ve oznitelik kacisi. E-posta istemcileri HTML calistirir;
 * kacissiz bir alan, alicinin gelen kutusuna istenen icerigi yerlestirir.
 */
...
/** Yalnizca http ve https; baska her sema null (javascript:, data: ...). */
function guvenliAdres(ham) {
  const metin = String(ham == null ? '' : ham).trim();
  if (!metin) return null;
  const aday = /^[a-zA-Z][a-zA-Z0-9+.-]*:/.test(metin) ? metin : `https://${metin}`;
  try {
...
/** Konu satiri HTML degil; yalnizca satir sonlari atilir (baslik enjeksiyonu). */
function tekSatir(deger) {
  return String(deger == null ? '' : deger).replace(/[\r\n]+/g, ' ').trim();
}

/**
 * 23 Eylul 2026: bu islevin CAGIRANI KALMADI. /notify-apply kaldirildi,
 * cunku her kullanicinin basvurusu operatorun kutusuna gidiyordu (K52).
 * Islev silinmedi, KACISLI hale getirildi: ozellik kullanicinin kendi
 * adresine gonderecek sekilde yeniden kurulursa bu sablon tekrar
 * kullanilacak ve eski, kacissiz hali K47'nin sunucu tarafi kopyasiydi:
 * `app.notes`, `app.role`, `app.company` dogrudan HTML'e yaziliyordu.
 */
async function sendApplicationNotification(app) {
  const subject = tekSatir(`📤 Yeni Başvuru: ${app.role} @ ${app.company}`);
  const adres   = guvenliAdres(app.url);
    ...
    <div style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;max-width:560px;margin:0 auto;background:#0d0f1a;color:#e2e8f0;border-radius:12px;overflow:hidden">
      ...
      <div style="padding:24px 28px">
        <table style="width:100%;border-collapse:collapse">
          <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px;width:120px">Şirket</td><td style="font-weight:600;font-size:15px;color:#f1f5f9">${kacis(app.company || '—')}</td></tr>
          <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Pozisyon</td><td style="font-weight:600;font-size:15px;color:#f1f5f9">${kacis(app.role || '—')}</td></tr>
          <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Tarih</td><td style="color:#f1f5f9">${kacis(app.date || new Date().toISOString().slice(0, 10))}</td></tr>
          <tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Durum</td><td><span style="background:#1e293b;border:1px solid #334155;padding:3px 10px;border-radius:6px;font-size:13px;color:#a5b4fc">${kacis(app.status || 'Başvuruldu')}</span></td></tr>
          ${app.notes ? `<tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">Notlar</td><td style="color:#cbd5e1;font-size:13px">${kacis(app.notes)}</td></tr>` : ''}
          ${adres ? `<tr><td style="padding:8px 0;color:#94a3b8;font-size:13px">İlan</td><td><a href="${kacis(adres)}" style="color:#818cf8;font-size:13px">${kacis(adres)}</a></td></tr>` : ''}
        </table>
...
module.exports = { sendMail, sendApplicationNotification, isConfigured, fromAddress, toAddress, _kacis: kacis, _guvenliAdres: guvenliAdres };
