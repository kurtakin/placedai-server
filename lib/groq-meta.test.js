...
test('/cues onMeta gecirip logluyor', () => {
  ...
  assert.match(aidSrc, /ag_ms/, 'log satirinda ag_ms yok');
});

// Groq'un total_time alani kuyrugu ICERMIYOR. Kuyrugu cikarmazsak ag
// olcusu kuyruk dalgalanmasini ag sanir: 6 Eylul'de bu hata ag'i 120-590 ms
// gosterdi, dogru formulle 34-47 ms cikti. Yanlis yon gosteren bir metrik
// hic olcmemekten kotu - bolge tasima karari buna bakiyordu.
test('ag_ms kuyrugu dusuyor (ms - kuyruk - groq)', () => {
  const i = aidSrc.indexOf('ag_ms:');
  assert.notStrictEqual(i, -1, 'ag_ms bulunamadi');
  const ifade = aidSrc.slice(i, i + 260);
  assert.match(ifade, /kuyruk_ms/,
    'ag_ms kuyrugu dusmuyor — kuyruk dalgalanmasi ag sanilir');
});
...
// Model katalogdan seciliyor ve katalog degisiyor (K10: llama-3.1-8b-instant
// bir donem dustu). Olcumun yaninda model adi olmazsa, bir sonraki katalog
// degisiminde ayni rakamlar sessizce baska bir modele ait olur ve kimse
// fark etmez.
test('timing hangi modelin olculdugunu soyluyor', () => {
  const i = aidSrc.indexOf('out.timing');
  const blok = aidSrc.slice(i, i + 400);
  assert.match(blok, /model:\s*meta\.model/,
    'timing icinde model yok — rakamlarin hangi modele ait oldugu bilinmiyor');
});
