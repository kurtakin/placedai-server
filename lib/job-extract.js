...
/**
 * Cevap KOD tasir, metin tasimaz.
 *
 * Ilk surumde sunucu hazir Turkce metin donuyordu ve kullanici arayuzu
 * Ingilizce'yken ekranda Turkce hata gorunuyordu (12 Eylul 2026, kullanicinin
 * ekran goruntusunde yakalandi). Sunucu kullanicinin dilini bilmez; metni
 * arayuz kendi sozlugunden yazmali (K25). `sebep` ve `oneri` yalnizca gunluk
 * ve eski cagiranlar icin duruyor.
 *
 * @returns {{ gecerli: boolean, kod: string, sebep: string, oneri: string }}
 */
function ilanCikarimiGecerliMi(jobData) {
  ...
  return { gecerli: true, kod: '', sebep: '', oneri: '' };
}
