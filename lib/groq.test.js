...
const parseCues = (() => {
  // Dilim DUSUNME_KALIBI'ndan basliyor: parseCues artik dusunmeMetni()'ni
  // cagiriyor ve sadece fonksiyon govdesini almak "dusunmeMetni is not
  // defined" veriyordu. Yardimcilar ayrilirsa dilim yine kirilir, o yuzden
  // baslangic sabit bir isaret degil, blogun kendi basi.
  const i = aidSrc.indexOf('const DUSUNME_KALIBI');
  assert.ok(i !== -1, 'DUSUNME_KALIBI bulunamadi — parseCies blogu tasinmis olabilir');
  const j = aidSrc.indexOf('function parseCues');
