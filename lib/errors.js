...
function getSupabase() {
  ...
  if (!url || !key) {
    console.warn('[errors] SUPABASE_URL / SUPABASE_SERVICE_ROLE_KEY missing — errors will only be logged');
    return null;
  }
...
async function logError(e = {}) {
  ...
  try {
    ...
    // Mail gönderimi isteği bekletmesin: ateşle ve unut.
    if (row.level === 'fatal') maybeAlert(row).catch(() => {});
    return { ok: true, fingerprint: row.fingerprint, first: !existing };
  } catch (err) {
    console.error('[errors] failed to record:', err.message);
...
async function maybeAlert(row) {
  ...
  if (!isConfigured()) {
    console.warn('[errors] alert skipped: email is not configured');
    _lastAlertAt.delete(row.fingerprint);
    return;
  }

  const res = await sendMail({
    to:      ALERT_TO,
    subject: `\u{1F534} PlacedAI error: ${String(row.message).slice(0, 80)}`,
    text: [
      `Source   : ${row.source}`,
  ...
  if (!res.ok) {
    console.error('[errors] could not send alert email:', res.error);
    // Gönderilemedi — bir sonraki denemeyi soğuma süresiyle engelleme.
    _lastAlertAt.delete(row.fingerprint);
  }
}
