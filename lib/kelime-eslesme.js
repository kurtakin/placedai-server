...
function terimGecer(terim, belgeNorm) {
  ...
  if (kelimeler.length > 1 &&
      kelimeler.every((w) => belgeNorm.includes(w) || belgeNorm.includes(tekille(w)))) {
...
function izVar(terim, belgeNorm) {
  ...
  return kelimeler.some((w) => belgeNorm.includes(w) || belgeNorm.includes(tekille(w)));
}
...
module.exports = { normalle, tekille, terimGecer, izVar, eslesmeleriDogrula };
