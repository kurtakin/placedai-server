 ...
 *
 * CANLI TEST (26 Eylul 2026): tek bir kisa cevap parcasi ("My email box and
 * customer requests") DORT kategorinin kaniti yapildi; konu disi sayilip
 * teknik bilgi, problem cozme ve role uyum "gelistirilmeli" cikti. Bir
 * parca iletisim hakkinda bir sey soyleyebilir, ama ayni parca dort ayri
 * yetenegi kanitlayamaz. Kural: bir alinti EN FAZLA BIR kategoriyi
 * destekler. Ayni alinti (ya da birinin digerini icermesi) ikinci kez
 * gelirse o kategori "degerlendirilemedi" olur. Oncelik KATEGORILER sirasi.
 */
...
function geriBildirimDenetle(r, cevaplar) {
  ...
  const categories = {};
  const kullanilan = [];   // sade() bicimiyle, bu turda kabul edilen alintilar
  for (const k of KATEGORILER) {
    ...
    let dogrulanamadi = false, tekrar = false;
    if (band !== 'not_assessable' && !alintiDogrula(evidence, cevaplar)) {
      band = 'not_assessable';
      dogrulanamadi = true;
    }
    if (band !== 'not_assessable') {
      const s = sade(evidence);
      if (kullanilan.some((u) => u.includes(s) || s.includes(u))) {
        band = 'not_assessable';
        tekrar = true;
      } else {
        kullanilan.push(s);
    ...
    if (band === 'not_assessable') { evidence = ''; if (dogrulanamadi || tekrar) comment = ''; }
    categories[k] = { band, comment, evidence, ...(dogrulanamadi ? { unverified: true } : {}), ...(tekrar ? { reused: true } : {}) };
  }
  return {
    categories,
