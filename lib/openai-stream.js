 ...
 */
// `ustveri`: anthropic.js ile ayni sozlesme. OpenAI'de kesilme isareti
...
async function createMessage({ model, max_tokens, system, messages }, ustveri) {
  ...
  if (ustveri && typeof ustveri === 'object') {
    ustveri.stop_reason = data.choices?.[0]?.finish_reason ?? null;
    ustveri.kesildi     = data.choices?.[0]?.finish_reason === 'length';
    ustveri.cikti_token = data.usage?.completion_tokens ?? null;
    ustveri.girdi_token = data.usage?.prompt_tokens ?? null;   // altin test seti maliyet olcumu (K61)
  }
