...
t('kapi: Ultimate ozellikleri sadece Ultimate\'de', () => {
  for (const f of ['performance', 'duo-mode', 'botapply']) {
...
t('kapi: eleme asamasi ucretli katmanlarda, free\'de degil', () => {
  // Coding ve online assessment ayni isin iki basamagi — ayni katmanda olmali.
  for (const f of ['coding', 'online-assessment']) {
    assert.strictEqual(planAllows('free', f),     false, f + ' free');
    assert.strictEqual(planAllows('pro', f),      true,  f + ' pro');
    assert.strictEqual(planAllows('ultimate', f), true,  f + ' ultimate');
  }
});
...
t('minPlanFor dogru katmani soyluyor', () => {
  ...
  assert.strictEqual(minPlanFor('online-assessment'), 'pro');
